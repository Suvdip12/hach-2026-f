...
(function () {
  ...
  const _imageCanvas = /**@type {HTMLCanvasElement}*/ (
    document.getElementById("imagecanvas")
  );
  const _imageCtx = /**@type {CanvasRenderingContext2D}*/ (
  ...
  const _main = () => {
     ...
     */
    const isUint32 = (x) =>
      typeof x == "number" && x == x >>> 0 && !isNegZero(x);
    ...
    const cmdBox = /**@type {HTMLInputElement}*/ (
    ...
    const listenClickById = (id, cb) => {
      ...
      elem && elem.addEventListener("click", cb);
    };
    ...
    [
      ...
      if (typeof eval(fn) !== "undefined") {
        globalThis[fn] = eval(fn);
      }
    });

    // Add backward as an alias
    globalThis.backward = function (distance) {
      forward(-distance);
    };
  ...
  // Only run if in browser environment and canvases exist
  if (typeof document !== "undefined") {
    // Wait for the canvas elements to be available
    const checkAndInit = () => {
      const turtleCanvas = document.getElementById("turtlecanvas");
      const imageCanvas = document.getElementById("imagecanvas");

      if (turtleCanvas && imageCanvas) {
        console.log("Turtle canvases found, initializing...");
        _main();
      } else {
        console.log("Waiting for turtle canvases...");
        // Try again after a short delay
        setTimeout(checkAndInit, 50);
      }
    };

    // Start checking immediately or wait for DOM ready
